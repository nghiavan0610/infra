...
show_help() {
    ...
    echo -e "  ${GREEN}logs-alertmgr${NC} Show Alertmanager logs"
...
check_health() {
    ...
    # Check Alertmanager
    if curl -s "http://localhost:${ALERTMANAGER_PORT:-9093}/-/healthy" > /dev/null 2>&1; then
...
show_urls() {
    ...
    echo -e "${GREEN}Alertmanager:${NC}    http://localhost:${ALERTMANAGER_PORT:-9093}"
    ...
    echo -e "${GREEN}OTLP gRPC:${NC}        localhost:${OTEL_GRPC_PORT:-4317}"
    echo -e "${GREEN}OTLP HTTP:${NC}        localhost:${OTEL_HTTP_PORT:-4318}"
    echo ""
}
...
backup_data() {
    ...
    # Backup volumes
    docker run --rm -v observability_prometheus_data:/data -v "$(pwd)/$backup_dir":/backup alpine tar czf /backup/prometheus-data.tar.gz -C /data .
    docker run --rm -v observability_loki_data:/data -v "$(pwd)/$backup_dir":/backup alpine tar czf /backup/loki-data.tar.gz -C /data .
    docker run --rm -v observability_tempo_data:/data -v "$(pwd)/$backup_dir":/backup alpine tar czf /backup/tempo-data.tar.gz -C /data .
    docker run --rm -v observability_grafana_data:/data -v "$(pwd)/$backup_dir":/backup alpine tar czf /backup/grafana-data.tar.gz -C /data .
    docker run --rm -v observability_alertmanager_data:/data -v "$(pwd)/$backup_dir":/backup alpine tar czf /backup/alertmanager-data.tar.gz -C /data .
